...
function asAmirror(str) {
  ...
  for(var i = 0; i < str.length; i++){
    ...
    if(str[i] === " " || i === str.length - 1){
      arrayLetra.push(temp);
      temp = "";
    }
  }

  for(var i = 0; i < arrayLetra.length; i++){
    for(var x = arrayLetra[i].length - 1; x >= 0; x--){
      fraseInv = fraseInv + arrayLetra[i][x];
    }

    if(i !== arrayLetra.length - 1){
      fraseInv = fraseInv + " ";
    }
  }
...
function capicua(numero){
  ...
  if(count === Math.ceil(numero.length/2)){
    return("Es capicua");
  } else {
